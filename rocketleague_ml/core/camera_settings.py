...
class Camera_Settings(Actor):
    ...
    def update_settings(self, updated_settings: Actor):
        ...
        if not attribute:
            raise ValueError(f"Attribute not found in {updated_settings.raw}")

        if updated_settings.secondary_category == "yaw":
            if "Byte" not in attribute:
                raise ValueError(f"Yaw not found with bytes {updated_settings.raw}")
        ...
        if updated_settings.secondary_category == "pitch":
            if "Byte" not in attribute:
                raise ValueError(f"Pitch not found with bytes {updated_settings.raw}")
            pitch_byte = attribute["Byte"]
            self.pitch = convert_byte_to_float(pitch_byte)
            return

        if updated_settings.secondary_category == "ball_cam":
            if "Boolean" not in attribute:
                raise ValueError(
                    f"Ball cam not found with boolean {updated_settings.raw}"
                )
            self.ball_cam = attribute["Boolean"]
            return

        if updated_settings.secondary_category == "rear_cam":
            if "Boolean" not in attribute:
                raise ValueError(
                    f"Rear cam not found with boolean {updated_settings.raw}"
                )
        ...
        if updated_settings.secondary_category == "camera_settings":
            if "CamSettings" not in attribute:
                raise ValueError(
                    f"Camera settings not found with settings {updated_settings.raw}"
                )
