...
def get_speed_cols(game: pd.DataFrame, teams: Dict[str, List[str]]):
    ...
    speed_cols["ball_positioning_linear_velocity"] = np.sqrt(
        (game["ball_positioning_linear_velocity_x"]) ** 2
        + (game["ball_positioning_linear_velocity_y"]) ** 2
        + (game["ball_positioning_linear_velocity_z"]) ** 2
    )

    speed_cols["ball_positioning_angular_velocity"] = np.sqrt(
        (game["ball_positioning_angular_velocity_x"]) ** 2
        + (game["ball_positioning_angular_velocity_y"]) ** 2
        + (game["ball_positioning_angular_velocity_z"]) ** 2
    )
...
def get_dependent_speed_cols(game: pd.DataFrame, teams: Dict[str, List[str]]):
    ...
    speed_cols["ball_is_semi_slow"] = (
        game["ball_positioning_linear_velocity"] > 500
    ) & (game["ball_positioning_linear_velocity"] <= 1000)
    speed_cols["ball_is_medium_speed"] = (
        game["ball_positioning_linear_velocity"] > 1000
    ) & (game["ball_positioning_linear_velocity"] <= 1500)
    speed_cols["ball_is_semi_fast"] = (
        game["ball_positioning_linear_velocity"] > 1500
    ) & (game["ball_positioning_linear_velocity"] <= 2000)
    speed_cols["ball_is_fast"] = game["ball_positioning_linear_velocity"] > 2000

    speed_cols["ball_is_drive_speed"] = game["ball_positioning_linear_velocity"] <= 1410
    speed_cols["ball_is_boost_speed"] = (
        game["ball_positioning_linear_velocity"] > 1410
    ) & (game["ball_positioning_linear_velocity"] < 2200)
    speed_cols["ball_is_supersonic"] = game["ball_positioning_linear_velocity"] >= 2200
